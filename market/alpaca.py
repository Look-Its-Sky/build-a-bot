...
if settings.keys.get('alpaca_api') and settings.keys.get('alpaca_secret'):
...
(Immediate Order)
'''
def buy_mo(pair: str, amount_in_USD: float):
    try:
        alpaca_api.submit_order(MarketOrderRequest(
            symbol = pair,
            side = OrderSide.BUY,
            time_in_force = TimeInForce.GTC,
            notional = amount_in_USD,
...
'''
def buy_lo(pair: str, limit_price: float, amount_in_USD: float):
...
(Immediate Order)
'''
def sell_mo(pair: str, amount_in_USD: float):
    try:
        alpaca_api.submit_order(MarketOrderRequest(
            symbol=pair,
            side=OrderSide.SELL,
...
(Preemptive)
'''
def sell_lo(pair: str, stop_price: float, stop_loss: float, limit_price: float, amount_in_USD: float):
    alpaca_api.submit_order(LimitOrderRequest(
        symbol=pair,
        side=OrderSide.SELL,
        time_in_force=TimeInForce.GTC,
